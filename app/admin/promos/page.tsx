  ...
  return (
    <>
      <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-20 flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(true)} className="bg-[#16A34A] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#15803d]">+ New Promo</button>
      </header>
              ...
              <div key={promo.id} className={`bg-white rounded-xl border shadow-sm p-5 ${promo.active ? "border-green-200" : "border-gray-200 opacity-60"}`}>
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => removePromo(promo.id)} className="text-green-400 hover:text-green-700">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input placeholder="e.g. Weekend Special 15% OFF" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A]" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Promo Code</label>
                <input placeholder="e.g. WEEKEND15" value={form.promoCode} onChange={(e) => setForm({ ...form, promoCode: e.target.value.toUpperCase().replace(/\s/g, "") })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A] uppercase tracking-wide" />
                <p className="text-[10px] text-gray-400 mt-1">Users enter this code at checkout to apply discount</p>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Description</label>
                <input placeholder="e.g. Valid on all laundry orders this weekend" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A]" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Applicable To</label>
                <select value={form.applicableTo} onChange={(e) => setForm({ ...form, applicableTo: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A]">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min={1} max={100} value={form.discountPercent || ""} onChange={(e) => setForm({ ...form, discountPercent: Number(e.target.value) })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A]" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 uppercase">Min Order (₱)</label>
                  <input type="number" min={0} value={form.minOrder || ""} onChange={(e) => setForm({ ...form, minOrder: Number(e.target.value) })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A]" />
                </div>
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-500 uppercase">Valid Until (optional)</label>
                <input type="date" value={form.validUntil} onChange={(e) => setForm({ ...form, validUntil: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-2.5 text-sm mt-2 outline-none focus:border-[#16A34A]" />
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={handleCreate} disabled={!form.title || !form.discountPercent} className="flex-1 bg-[#16A34A] text-white py-2.5 rounded-lg text-sm font-bold disabled:opacity-40">Create Promo</button>
              </div>
            </div>
          </div>
        </div>
