...
const STATUS_COLORS: Record<string, string> = {
  ...
  resolved: "bg-[#59EBC6]/20 text-green-800",
}
    ...
    <>
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm">
            <p className="text-2xl font-bold text-[#319F44]">{reports.filter((r) => r.status === "resolved").length}</p>
          ...
          {(["all", "pending", "reviewed", "resolved"] as const).map((s) => (
            <button
              ...
              className={`px-4 py-2 text-xs rounded-lg capitalize font-medium transition-colors ${
                filter === s ? "bg-[#319F44] text-white" : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
            >
              {s}
            </button>
                ...
                <div className="p-5">
                  ...
                  <div className="flex gap-2 pt-3 border-t border-gray-100">
                    ...
                    {report.status !== "resolved" && (
                      <button onClick={() => handleStatus(report.id, "resolved")} className="text-xs bg-[#319F44]/100 text-white px-3 py-1.5 rounded-lg font-medium hover:bg-green-600">
                        ✓ Resolve
                      </button>
                    )}
                  </div>
