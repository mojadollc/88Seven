...
const SERVICES = [
  { id: "grocery", label: "Grocery", link: "/grocery", color: "from-[#319F44] via-[#64DEA7] to-[#59EBC6]", icon: "🛒" },
...
export default function HeroImagesAdmin() {
  ...
  async function handleSave(serviceId: string) {
    ...
    if (existing) {
      ...
      const res = await fetch("/api/hero", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ badge: svc.label, title: svc.label, highlight: "", description: "", imageUrl, bgColor: "#319F44", link: svc.link, order: 0, enabled: true }) })
            ...
            <div key={svc.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                ...
                <button onClick={() => fileRefs.current[svc.id]?.click()} disabled={isUploading}
                  className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-gray-200 hover:border-[#319F44] rounded-lg py-2 text-sm text-gray-500 hover:text-[#319F44] transition-colors disabled:opacity-50">
                ...
                <input type="text" value={currentImage} onChange={e => setUrls(p => ({ ...p, [svc.id]: e.target.value }))}
                  placeholder="Or paste image URL..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-[#319F44]" />

                <button onClick={() => handleSave(svc.id)} disabled={isSaving}
                  className="w-full bg-[#319F44] text-white text-sm font-semibold py-2 rounded-lg hover:bg-[#267a34] disabled:opacity-50 transition-colors">
