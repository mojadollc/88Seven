...
export default function AdminBannersPage() {
  ...
  const [form, setForm] = useState({ title: "", subtitle: "", imageUrl: "", bgColor: "#319F44", link: "/grocery", order: 0, enabled: true })
  ...
  const handleSave = async () => {
    ...
    setForm({ title: "", subtitle: "", imageUrl: "", bgColor: "#319F44", link: "/grocery", order: 0, enabled: true })
    await loadBanners()
  }
    ...
    <>
      <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-20">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { setEditing(null); setForm({ title: "", subtitle: "", imageUrl: "", bgColor: "#319F44", link: "/grocery", order: banners.length + 1, enabled: true }); setShowForm(true) }} className="text-xs bg-[#319F44] text-white px-4 py-2 rounded-lg font-bold">+ Add Banner</button>
        </div>
      </header>
                  ...
                  <div className="flex-1 p-4 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${banner.enabled ? "bg-[#59EBC6]/20 text-[#267a34]" : "bg-gray-100 text-gray-500"}`}>{banner.enabled ? "Active" : "Disabled"}</span>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleToggle(banner)} className={`w-10 h-5 rounded-full transition-colors ${banner.enabled ? "bg-[#319F44]/100" : "bg-gray-300"}`}>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-[#319F44]" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500">Subtitle</label>
                  <input value={form.subtitle} onChange={(e) => setForm({ ...form, subtitle: e.target.value })} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-[#319F44]" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  ...
                  <input value={form.link} onChange={(e) => setForm({ ...form, link: e.target.value })} placeholder="/grocery or https://... (leave empty for no link)" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1 outline-none focus:border-[#319F44]" />
                  <div className="flex gap-1 mt-1.5">
                    {["/grocery", "/laundry", "/services", "/travel"].map((l) => (
                      <button key={l} type="button" onClick={() => setForm({ ...form, link: l })} className={`text-[10px] px-2 py-0.5 rounded border ${form.link === l ? "bg-[#319F44] text-white border-[#319F44]" : "bg-gray-50 text-gray-500 border-gray-200"}`}>{l.replace("/", "")}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t flex justify-end gap-3">
              ...
              <button onClick={handleSave} className="bg-[#319F44] text-white px-5 py-2 rounded-lg text-sm font-bold">Save</button>
            </div>
          </div>
        </div>
      )}
