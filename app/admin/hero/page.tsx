...
const TABS = [
  { key: "home", label: "🏠 Home Page Slider", hint: "Shown on the main homepage hero section" },
  { key: "grocery", label: "🛒 Grocery Page Slider", hint: "Shown inside the grocery page banner" },
]

export default function AdminHero() {
  const [tab, setTab] = useState<"home" | "grocery">("home")
  const [slides, setSlides] = useState<Record<string, any[]>>({ home: [], grocery: [] })
  ...
  function handleNew() {
    setEditing(null)
    setForm({ ...defaultSlide, order: (slides[tab]?.length || 0) + 1 })
    setShowForm(true)
  }
  ...
  async function handleToggle(slide: any) {
    ...
    setSlides(prev => ({
      ...prev,
      [tab]: prev[tab].map(s => s.id === slide.id ? { ...s, enabled: !s.enabled } : s)
    }))
  }

  const currentSlides = slides[tab] || []
    ...
    <>
      <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-20">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-[#1F2937]">Hero Slides</h1>
            <p className="text-xs text-gray-400 mt-0.5">Manage sliders independently for each page</p>
          </div>
        ...
        <div className="flex gap-1 mt-4">
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key as any)}
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab === t.key ? "bg-[#319F44] text-white" : "text-gray-500 hover:bg-gray-100"}`}>
              {t.label}
            </button>
          ))}
        </div>
        <p className="text-[11px] text-gray-400 mt-2">{TABS.find(t => t.key === tab)?.hint}</p>
      </header>
      ...
      <div className="p-6">
              ...
              <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {editing ? `Editing — ${tab === "home" ? "Home Page" : "Grocery Page"}` : `Adding to — ${tab === "home" ? "Home Page" : "Grocery Page"} slider`}
                  </p>
                </div>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="rounded-lg overflow-hidden relative min-h-[120px] flex items-end text-white" style={{ backgroundColor: form.bgColor }}>
                  ...
                  <div className="relative z-10 p-5 w-full" style={{ background: form.imageUrl ? "linear-gradient(to top, rgba(0,0,0,0.55) 0%, transparent 100%)" : "none" }}>
                    ...
                    <p className="text-white/80 text-sm mt-1 drop-shadow">{form.description || "Description"}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mt-1">
                    {tab === "home" ? "Recommended: 1440 × 560px landscape" : "Recommended: 900 × 400px landscape"} • JPG or PNG
                  </p>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Link URL (optional)</label>
                  <input type="text" value={form.link} onChange={e => setForm({ ...form, link: e.target.value })} placeholder="/grocery or https://..." className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#319F44]" />
                </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-10 text-center">
            <p className="text-gray-500 font-medium">No slides yet for {tab === "home" ? "Home Page" : "Grocery Page"}</p>
            <p className="text-gray-400 text-sm mt-1">Click &quot;+ Add Slide&quot; to create your first slide</p>
          </div>
        ) : (
            ...
            {currentSlides.map(slide => (
              <div key={slide.id} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex flex-col md:flex-row">
                  <div className="w-full md:w-[280px] relative min-h-[100px] text-white flex-shrink-0 flex items-end overflow-hidden" style={{ backgroundColor: slide.bgColor }}>
                  ...
                  <div className="flex-1 p-5 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${slide.enabled ? "bg-[#319F44]/10 text-[#267a34]" : "bg-gray-100 text-gray-500"}`}>
                          {slide.enabled ? "Active" : "Disabled"}
                        </span>
                        <span className="text-[10px] text-gray-400">Order: {slide.order}</span>
                      </div>
                      <p className="font-medium text-sm text-[#1F2937]">{slide.title} {slide.highlight && `— ${slide.highlight}`}</p>
                      {slide.description && <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{slide.description}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleToggle(slide)} className={`relative w-11 h-6 rounded-full transition-colors ${slide.enabled ? "bg-[#319F44]" : "bg-gray-300"}`}>
                      ...
                      <button onClick={() => handleDelete(slide.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
