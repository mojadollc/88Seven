...
export default function AdminHero() {
  ...
  const [form, setForm] = useState<Omit<HeroSlide, "id">>(defaultSlide)
    ...
    <>
      <header className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-20">
        <div className="flex items-center justify-between">
          <h1 className="text-lg font-bold text-[#1a1a2e]">Hero Slides Management</h1>
          <button onClick={handleNew} className="bg-[#D62828] text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-[#b71c1c] transition-colors">+ Add Slide</button>
        </div>
      </header>
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-xl">
              <div className="p-6 border-b border-gray-100">
                <h2 className="font-bold text-lg text-[#1a1a2e]">{editing ? "Edit Slide" : "New Slide"}</h2>
              </div>
              <div className="p-6 space-y-4">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })} placeholder="e.g. Express Delivery" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828]" />
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <input type="text" value={form.bgColor} onChange={(e) => setForm({ ...form, bgColor: e.target.value })} className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828]" />
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <input type="text" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Fast Delivery" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828]" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Highlight Text</label>
                  <input type="text" value={form.highlight} onChange={(e) => setForm({ ...form, highlight: e.target.value })} placeholder="e.g. To Your Doorstep" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828]" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">Description</label>
                  <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Short description" rows={2} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828] resize-none" />
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={form.order} onChange={(e) => setForm({ ...form, order: parseInt(e.target.value) || 0 })} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828]" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-600 mb-1">Status</label>
                    <select value={form.enabled ? "enabled" : "disabled"} onChange={(e) => setForm({ ...form, enabled: e.target.value === "enabled" })} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#D62828]">
              ...
              <div className="p-6 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button onClick={handleSave} disabled={saving} className="bg-[#D62828] text-white text-sm font-medium px-5 py-2 rounded-lg hover:bg-[#b71c1c] disabled:opacity-50">{saving ? "Saving..." : editing ? "Update" : "Create"}</button>
              </div>
            </div>
          </div>
        )}
                ...
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="flex-1 p-5 flex items-center justify-between">
                    <div>
                      ...
                      <p className="font-medium text-sm text-[#1a1a2e]">{slide.title} — {slide.highlight}</p>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => handleEdit(slide)} className="p-2 text-gray-400 hover:text-[#D62828] hover:bg-red-50 rounded-lg transition-colors">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                      </button>
                      <button onClick={() => handleDelete(slide.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
