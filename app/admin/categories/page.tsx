  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowAdd(true)} className="bg-[#319F44] text-white px-4 py-2 rounded-lg text-sm font-medium">+ Add Category</button>
      </div>
          ...
          {categories.map((cat) => (
            <div key={cat.id} className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="flex gap-2">
                ...
                <label className="flex-1 text-xs bg-[#319F44]/10 text-[#319F44] py-1.5 rounded-lg font-medium text-center cursor-pointer">
                  {uploading ? "..." : "Image"}
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => { if (e.target.files?.[0]) handleImageUpload(cat.id, e.target.files[0]) }} />
                </label>
                <button onClick={() => handleDelete(cat.id)} className="text-xs bg-[#319F44]/10 text-[#267a34] py-1.5 px-3 rounded-lg font-medium">✕</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleSave} className="flex-1 bg-[#319F44] text-white py-2 rounded-lg text-sm font-bold">Save</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl w-full max-w-sm p-6 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={handleAdd} className="flex-1 bg-[#319F44] text-white py-2 rounded-lg text-sm font-bold">Add</button>
            </div>
          </div>
        </div>
      )}
