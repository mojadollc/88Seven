...
export default function AccountPage() {
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="w-10 h-10 border-4 border-[#319F44] border-t-transparent rounded-full animate-spin" />
    </div>
  )

  if (!user) return (
    <div className="min-h-screen bg-gradient-to-b from-[#319F44] to-[#267a34] flex items-center justify-center p-4">
      <div className="text-center w-full max-w-sm">
        ...
        <a href="/auth?redirect=/account" className="block w-full bg-white text-[#319F44] px-6 py-3.5 rounded-xl text-sm font-bold hover:bg-gray-100 transition-colors shadow-lg">
    ...
    <main className="min-h-screen pb-20" style={{ backgroundColor: "var(--theme-page-bg, #F5F5DB)" }}>
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white mx-4 mt-4 rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-[#319F44] to-[#319F44] px-5 pt-6 pb-10">
          ...
          <div className="mx-4 -mt-6 relative">
            <div className="bg-white rounded-xl shadow-md border border-gray-100 grid grid-cols-3 divide-x divide-gray-100">
              <div className="p-3 text-center">
                <p className="text-lg font-bold text-[#319F44]">{totalActive}</p>
              ...
              <div className="p-3 text-center">
                <p className="text-lg font-bold text-[#319F44]">₱{totalSpent.toFixed(0)}</p>
        ...
        {/* Wallet Card */}
        <a href="/account/wallet" className="block mx-4 mt-4 bg-gradient-to-r from-green-600 to-[#59EBC6] rounded-xl p-4 text-white shadow-sm">
          ...
          <div className="px-4 pt-4 pb-8">
            ...
            <div className="flex gap-2 mb-4">
              {(["active", "completed", "all"] as const).map((f) => (
                <button key={f} onClick={() => setOrderFilter(f)} className={`px-3 py-1.5 text-[11px] rounded-lg font-medium capitalize transition-colors ${orderFilter === f ? "bg-[#319F44] text-white" : "bg-white border border-gray-200 text-gray-500"}`}>
              ...
              {getFilteredOrders().length === 0 ? (
                <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
                  ...
                  <div className="flex gap-3 justify-center mt-5">
                    <a href="/grocery" className="text-xs bg-[#319F44] text-white px-4 py-2.5 rounded-lg font-bold">Shop Grocery</a>
                  ...
                  return (
                    <a key={`${order.type}-${order.id}`} href={href} className="block bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                      <div className="p-4 flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${isGrocery ? "bg-[#319F44]/10" : "bg-blue-50"}`}>
                          {isGrocery ? (
                            <svg className="w-5 h-5 text-[#319F44]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" /></svg>
                          ) : (
                        ...
                        <div className="text-right shrink-0">
                          <p className={`text-sm font-bold ${isGrocery ? "text-[#319F44]" : "text-blue-600"}`}>₱{order.amount}</p>
          ...
          <div className="px-4 pt-4 pb-8 space-y-3">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="divide-y divide-gray-50">
                ...
                <button onClick={() => { clearAuth(); window.location.href = "/" }} className="w-full px-5 py-3.5 flex items-center gap-4 hover:bg-[#319F44]/10 transition-colors text-left">
                  <img src="https://img.icons8.com/3d-fluency/94/exit.png" alt="Logout" className="w-8 h-8 object-contain shrink-0" />
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#267a34]">Logout</p>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 safe-bottom">
        <div className="max-w-lg mx-auto grid grid-cols-4 py-1.5">
          ...
          <a href="/account" className="flex flex-col items-center gap-0.5 py-1 text-[#319F44]">
