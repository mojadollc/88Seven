                  ...
                  <div className="p-4">
                    ...
                    {job.status === "pending" && (
                      <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                        <button onClick={() => handleRejectJob(job.id)} className="flex-1 border border-green-200 text-green-700 py-2.5 rounded-lg text-xs font-bold hover:bg-green-50">Decline</button>
                        {isOnline && (
                    ...
                    {job.status === "completed" && <p className="text-xs text-green-600 font-medium mt-3 pt-3 border-t border-gray-100">✓ Completed</p>}
                  </div>
                </div>
              ))
            )}
                  ...
                  <div className="divide-y divide-gray-50">
                    {transactions.slice(0, 5).map((txn) => (
                      <div key={txn.id} className="px-4 py-3 flex items-center justify-between">
                        <div>
                          <p className={`text-xs font-semibold ${txn.type === "topup" ? "text-blue-600" : txn.type === "earning" ? "text-green-600" : "text-green-500"}`}>
