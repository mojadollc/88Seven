...
import { onCustomerAuthChange, getCustomerWalletBalance, getCustomerWalletTransactions, topUpCustomerWallet, type CustomerWalletTransaction } from "@/lib/firebase"
...
export default function WalletPage() {
    ...
    const unsub = onCustomerAuthChange(async (u) => {
      ...
      if (u) {
        const [bal, txns] = await Promise.all([
          getCustomerWalletBalance(u.uid),
          getCustomerWalletTransactions(u.uid),
        ])
        setBalance(bal)
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      const payment = await createXenditPayment({
        ...
        externalId: `wallet_topup_${user.uid}_${Date.now()}`,
  ...
  useEffect(() => {
    ...
    if (params.get("topup") === "success" && user) {
      ...
      if (amount > 0) {
        topUpCustomerWallet(user.uid, amount, `topup_${Date.now()}`).then(() => {
          setBalance((b) => b + amount)
          getCustomerWalletTransactions(user.uid).then(setTransactions)
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 grid grid-cols-3 gap-3">
          ...
          <a href="/grocery" className="flex flex-col items-center gap-1.5">
            <div className="w-10 h-10 bg-red-100 rounded-full flex items-center justify-center">
              <svg className="w-5 h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" /></svg>
            </div>
              ...
              <div key={txn.id} className="px-4 py-3 flex items-center gap-3">
                <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${txn.amount >= 0 ? "bg-green-100" : "bg-red-100"}`}>
                  {txn.amount >= 0 ? (
                    ...
                    <svg className="w-4 h-4 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 12H4" /></svg>
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-800 truncate">{txn.note || (txn.amount >= 0 ? "Top-up" : "Payment")}</p>
                  <p className="text-[10px] text-gray-400">{txn.createdAt ? new Date(txn.createdAt.seconds * 1000).toLocaleDateString("en-PH", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : ""}</p>
                </div>
