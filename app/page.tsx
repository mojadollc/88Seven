...
const SERVICES = [
  { id: "grocery", name: "Grocery", icon: "🛒", href: "/grocery", available: true, color: "from-[#4194AF] via-[#61B288] to-[#93D569]", desc: "Fresh produce & daily essentials", badge: "Same-day delivery" },
...
export default function HomePage() {
  ...
  useEffect(() => {
    fetch("/api/hero").then(r => r.ok ? r.json() : []).then((data: any[]) => {
      setBanners(data.length > 0 ? data.map((s: any) => ({ id: s.id, title: s.title || "", subtitle: s.description || s.subtitle || "", imageUrl: s.imageUrl || "", bgColor: s.bgColor || "#4194AF", link: s.link || "/grocery" })) : [
        { id: "1", title: "Free Delivery", subtitle: "On orders ₱1,000+", imageUrl: "", bgColor: "#4194AF", link: "/grocery" },
      ...
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <a href="/" className="flex items-center gap-2 shrink-0">
            <div className="w-8 h-8 bg-[#4194AF] rounded-xl flex items-center justify-center shadow-sm">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV_ITEMS.map(item => (
              <a key={item.label} href={item.href} className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-[#4194AF] hover:bg-[#4194AF]/10 rounded-lg transition-colors">{item.label}</a>
            ))}
          </nav>

{/* Right actions */}
          <div className="flex items-center gap-2">
            <button onClick={() => setShowAddressModal(true)} className="hidden md:flex items-center gap-1.5 text-sm text-gray-600 hover:text-[#4194AF] px-3 py-2 rounded-lg hover:bg-gray-50 transition-colors">
              <svg className="w-4 h-4 text-[#4194AF] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
              ...
              {showNotifs && (
                <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b flex items-center justify-between">
                    ...
                    {mounted && unread > 0 && user && <button onClick={markAllRead} className="text-xs text-[#4194AF] font-semibold">Mark all read</button>}
                  </div>
                  <div className="max-h-64 overflow-y-auto">
                    {notifications.length === 0 ? <p className="p-6 text-sm text-gray-400 text-center">No notifications yet</p> : notifications.slice(0, 8).map(n => (
                      <div key={n.id} className={`px-4 py-3 border-b border-gray-50 ${!n.read ? "bg-[#4194AF]/10" : ""}`}>
            ...
            <a href="/account" className="hidden md:flex items-center gap-2 bg-[#4194AF] hover:bg-[#3a7d96] text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white px-4 py-3 space-y-1">
            {NAV_ITEMS.map(item => (
              <a key={item.label} href={item.href} className="block px-3 py-2.5 text-sm font-medium text-gray-700 hover:text-[#4194AF] hover:bg-[#4194AF]/10 rounded-xl transition-colors">{item.label}</a>
            ))}
            <div className="pt-2 border-t border-gray-100 mt-2">
              ...
              <a href="/account" className="flex items-center gap-2 px-3 py-2.5 text-sm font-semibold text-[#4194AF] hover:bg-[#4194AF]/10 rounded-xl">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
      ...
      <section className="relative min-h-[560px] md:min-h-[640px] flex items-center border-b border-gray-100 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 py-12 md:py-20 w-full">
          <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
            ...
            {/* LEFT: Text content */}
            <div>
              <div className="inline-flex items-center gap-2 bg-[#93D569]/20 text-[#4194AF] text-xs font-bold px-3 py-1.5 rounded-full mb-5">
                <span className="w-1.5 h-1.5 bg-[#4194AF] rounded-full animate-pulse" />
              ...
              <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-gray-900 leading-tight tracking-tight">
                Everything<br />
                <span className="text-[#4194AF]">delivered</span><br />
                to your door.
              </h1>
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-3">
                <button onClick={() => setShowAddressModal(true)} className="flex items-center gap-2 bg-white border-2 border-gray-200 hover:border-[#4194AF] rounded-xl px-4 py-3 text-sm text-gray-600 transition-colors flex-1 sm:flex-none sm:min-w-[200px]">
                  <svg className="w-4 h-4 text-[#4194AF] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                  <span className="truncate">{detecting ? "Detecting..." : address || "Set your location"}</span>
                </button>
                <a href="/grocery" className="flex items-center justify-center gap-2 bg-[#4194AF] hover:bg-[#3a7d96] text-white font-bold px-6 py-3 rounded-xl transition-colors shadow-lg shadow-[#93D569]/30 text-sm">
                  Order Now
          ...
          <div className="flex gap-1.5 mt-8">
            {SERVICES.map((_, i) => (
              <button key={i} onClick={() => setActiveService(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeService ? "bg-[#4194AF] w-5" : "bg-gray-300 w-1.5"}`} />
            ))}
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
          {SERVICES.map((s, i) => (
            <a key={s.id} href={s.available ? s.href : undefined}
              className={`relative group bg-white rounded-2xl border border-gray-100 p-4 md:p-5 flex flex-col items-center gap-3 shadow-sm transition-all ${s.available ? "hover:shadow-lg hover:-translate-y-1 hover:border-[#4194AF]/30 cursor-pointer" : "opacity-50 cursor-default"}`}>
      ...
      {mounted && banners.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 md:px-8 pb-12">
          <div className="relative rounded-3xl overflow-hidden h-[180px] md:h-[300px] shadow-lg cursor-pointer"
            style={{
              backgroundColor: banners[currentBanner]?.bgColor || "#4194AF",
      ...
      {mounted && promos.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 md:px-8 pb-12">
          <div className="flex items-center justify-between mb-5">
            ...
            <span className="text-xs text-[#4194AF] font-semibold bg-[#4194AF]/10 px-3 py-1 rounded-full">Limited time</span>
          </div>
            ...
            {promos.map((p) => (
              <div key={p.id} className="flex-shrink-0 min-w-[220px] md:min-w-0 bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-all hover:-translate-y-0.5">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-gradient-to-br from-[#4194AF] to-[#93D569] rounded-xl flex items-center justify-center shrink-0 shadow-sm">
      ...
      {mounted && partners.length > 0 && (
        <section className="max-w-7xl mx-auto px-4 md:px-8 pb-12">
          <div className="flex items-center justify-between mb-5">
            ...
            <a href="/laundry" className="text-xs text-[#4194AF] font-semibold hover:underline">See all</a>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 md:gap-8">
            {[
              ...
              <div key={s.step} className="bg-white rounded-2xl p-5 md:p-8 border border-gray-100 shadow-sm text-center hover:shadow-md transition-all hover:-translate-y-0.5">
                ...
                <span className="text-[10px] font-black text-[#4194AF] tracking-widest">{s.step}</span>
      ...
      <footer className="max-w-7xl mx-auto px-4 md:px-8 py-10 md:py-12">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-[#4194AF] rounded-xl flex items-center justify-center">
              <span className="text-white font-black text-sm">G</span>
            </div>
            <span className="font-black text-xl text-gray-900 tracking-tight">Gruwcer</span>
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            {[["Grocery", "/grocery"], ["Laundry", "/laundry"], ["Services", "/home-services"], ["Account", "/account"], ["Sign In", "/auth"]].map(([label, href]) => (
              <a key={label} href={href} className="text-sm text-gray-400 hover:text-[#4194AF] transition-colors">{label}</a>
            ))}
          </div>
        </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-50 safe-bottom" suppressHydrationWarning>
        <div className="grid grid-cols-4 py-2">
          <a href="/" className="flex flex-col items-center gap-0.5 py-1 text-[#4194AF]">
          ...
          {!showOrderTracker && (
            <button onClick={() => setShowOrderTracker(true)} className="fixed bottom-20 md:bottom-8 left-1/2 -translate-x-1/2 z-[60] bg-white border border-gray-200 shadow-2xl rounded-full pl-14 pr-5 py-3 flex items-center gap-3 hover:shadow-xl transition-shadow">
              <div className="absolute left-2 top-1/2 -translate-y-1/2 flex">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#4194AF] to-[#93D569] border-2 border-white flex items-center justify-center z-30 shadow-md text-lg">🛒</div>
              ...
              <div className="w-5 h-5 bg-[#4194AF] rounded-full flex items-center justify-center ml-1">
          ...
          {showOrderTracker && (
            <div className="fixed bottom-20 md:bottom-8 left-4 right-4 md:left-auto md:right-8 md:w-96 z-[60] bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-[#4194AF] rounded-full animate-pulse" />
                    ...
                    <a key={`${order.type}-${order.id}`} href={href} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1.5 h-1 bg-gray-100 rounded-full overflow-hidden">
                          <div className="h-full bg-[#4194AF] rounded-full transition-all duration-500" style={{ width: `${progress}%` }} />
                        </div>
              ...
              <div className="px-4 py-2.5 bg-gray-50 border-t border-gray-100">
                <a href="/account" className="text-xs text-[#4194AF] font-bold text-center block">View all orders →</a>
              </div>
            </div>
          )}
        </>
        ...
        <div className="fixed inset-0 z-[200] flex flex-col justify-end md:items-center md:justify-center">
          ...
          <div className="relative bg-white rounded-t-3xl md:rounded-2xl w-full max-w-lg mx-auto overflow-hidden">
            ...
            <div className="px-5 py-4 max-h-[75vh] overflow-y-auto space-y-3">
              <button onClick={() => { detectLocation(); setShowAddressModal(false) }} className="w-full flex items-center gap-3 bg-[#4194AF]/10 border border-[#4194AF]/20 rounded-xl px-4 py-3 hover:bg-[#93D569]/20 transition-colors">
                <div className="w-9 h-9 bg-[#4194AF] rounded-xl flex items-center justify-center shrink-0">
                  <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                </div>
                <div className="text-left">
                  <p className="text-sm font-bold text-[#4194AF]">Use current location</p>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {savedAddresses.map((addr) => {
                      ...
                      return (
                        <div key={addr.id} onClick={() => selectAddress(addr)} className={`flex items-center gap-3 border rounded-xl px-4 py-3 cursor-pointer transition-colors ${active ? "border-[#4194AF] bg-[#4194AF]/10" : "border-gray-200 hover:border-gray-300"}`}>
                          ...
                          {active && <div className="w-2 h-2 bg-[#4194AF] rounded-full shrink-0" />}
              ...
              {!addingNew ? (
                <button onClick={() => setAddingNew(true)} className="w-full flex items-center gap-3 border-2 border-dashed border-gray-200 rounded-xl px-4 py-3 hover:border-[#4194AF] transition-colors">
                ...
                <div className="border border-gray-200 rounded-xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {(["Home", "Office", "Work", "Other"] as const).map((lbl) => {
                      ...
                      return (
                        <button key={lbl} onClick={() => setNewAddr((p) => ({ ...p, label: lbl }))} className={`py-2 rounded-xl text-xs font-bold border transition-colors ${newAddr.label === lbl ? "bg-[#4194AF] text-white border-[#4194AF]" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"}`}>
                          <span className="block text-base mb-0.5">{lblIcons[lbl]}</span>{lbl}
                        </button>
                      )
                    })}
                  </div>
                  <textarea placeholder="Enter full address..." value={newAddr.address} onChange={(e) => setNewAddr((p) => ({ ...p, address: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-[#4194AF] resize-none" rows={2} />
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={saveNewAddress} disabled={savingAddr || !newAddr.address.trim()} className="flex-1 bg-[#4194AF] text-white py-2.5 rounded-xl text-xs font-bold disabled:opacity-40">{savingAddr ? "Saving..." : "Save Address"}</button>
                  </div>
                </div>
              )}
              {mounted && !user && <p className="text-xs text-center text-gray-400 pb-2"><a href="/auth" className="text-[#4194AF] font-bold">Sign in</a> to save addresses across devices</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {mounted && showInstall && !isStandalone && (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl">
            <div className="bg-gradient-to-br from-[#4194AF] to-[#3a7d96] px-6 pt-8 pb-10 text-center relative overflow-hidden">
              <div className="absolute -right-8 -top-8 w-32 h-32 bg-white/5 rounded-full" />
              <div className="w-14 h-14 bg-white rounded-2xl shadow-xl flex items-center justify-center mx-auto mb-4">
                <div className="w-8 h-8 bg-[#4194AF] rounded-lg flex items-center justify-center"><span className="text-white font-black text-sm">G</span></div>
              </div>
