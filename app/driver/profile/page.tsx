      ...
      <div className="max-w-lg mx-auto p-4 space-y-4 pb-24">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <div className="space-y-3">
            <select value={form.vehicleType} onChange={(e) => setForm({ ...form, vehicleType: e.target.value })} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#319F44]">
              <option value="motorcycle">Motorcycle</option>
              <option value="ebike">E-Bike</option>
              <option value="bicycle">Bicycle</option>
            </select>
            <input placeholder="Plate Number" value={form.plateNumber} onChange={(e) => setForm({ ...form, plateNumber: e.target.value.toUpperCase() })} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#319F44]" />
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          ...
          <input type="tel" placeholder="09xxxxxxxxx" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value.replace(/[^0-9]/g, "") })} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-[#319F44]" />
        </div>
      </div>

      {/* Save Button */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 max-w-lg mx-auto">
        <button onClick={handleSave} disabled={saving} className="w-full bg-[#319F44] text-white py-3.5 rounded-xl font-bold text-sm hover:bg-[#267a34] transition-colors disabled:opacity-50">
