  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-lg mx-auto p-4 space-y-4">
        {/* Balance Card */}
        <div className="bg-gradient-to-r from-green-600 to-[#93D569] rounded-2xl p-6 text-white">
          ...
          <button onClick={() => setShowTopUp(true)} className="mt-4 bg-white text-[#3a7d96] px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-[#4194AF]/10 transition-colors">
            + Top Up Wallet
          </button>
        </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-2">
                  {[100, 200, 500].map((amt) => (
                    <button key={amt} onClick={() => setTopUpAmount(amt)} className={`py-2.5 rounded-lg text-sm font-bold border transition-colors ${topUpAmount === amt ? "border-green-600 bg-[#4194AF]/10 text-[#3a7d96]" : "border-gray-200 text-gray-600"}`}>
                      ₱{amt}
                    </button>
                  ))}
                </div>
