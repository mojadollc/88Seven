...
function FailedContent() {
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true)
    try {
      const col = collectionMap[service] || "orders"
      await updateDoc(doc(db, col, id), { status: "cancelled" })
      setCancelled(true)
    } catch (e) {
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="text-center max-w-sm w-full">
          ...
          <a href="/" className="block bg-[#D62828] text-white px-6 py-3 rounded-xl font-bold text-sm">
            Go to Home
          </a>
        </div>
      </main>
    ...
    <main className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="text-center max-w-sm w-full">
        ...
        <div className="space-y-3">
          <a href={`/${service}`} className="block bg-[#D62828] text-white px-6 py-3 rounded-xl font-bold text-sm">
            Try Again
          </a>
          <button
