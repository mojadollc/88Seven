...
export default function GroceryPage() {
  ...
  return (
    ...
    <main className="min-h-screen bg-gray-50 pb-20">
      ...
      {/* ── HEADER ── */}
      <header className="bg-[#16A34A] sticky top-0 z-50 shadow-sm">
              ...
              {showNotifications && (
                <div className="absolute right-0 top-10 w-72 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                  <div className="px-4 py-3 border-b flex items-center justify-between">
                    ...
                    {notifications.filter(n => !n.read).length > 0 && <button onClick={() => fetch("/api/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "markAllRead", recipientType: "customer", recipientId: user.id }) }).then(() => setNotifications(n => n.map(x => ({ ...x, read: true }))))} className="text-xs text-[#16A34A] font-semibold">Mark all read</button>}
                  </div>
        ...
        <div className="mt-3 bg-white rounded-2xl shadow-sm border border-gray-100 px-3 py-4">
          <div className="flex gap-2 overflow-x-auto scrollbar-hide">
            {allCategories.map((cat) => {
              ...
              const colorClass = isActive ? "bg-[#16A34A] text-white border-[#16A34A]" : `${getCatColor(cat)} border-transparent`
              return (
                <button
                  key={cat}
        ...
        <div className="mt-3">
          <div className="flex items-center justify-between mb-2">
            ...
            {selectedCategory !== "All" && (
              <button onClick={() => setSelectedCategory("All")} className="text-xs text-[#16A34A] font-semibold">Clear filter</button>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl p-16 text-center border border-gray-100">
              ...
              <button onClick={() => { setSelectedCategory("All"); setSearchQuery("") }} className="mt-3 text-sm text-[#16A34A] font-semibold">Clear filters</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-3">
          ...
          <div className="relative w-full max-w-md bg-white h-full flex flex-col shadow-2xl">
            ...
            {cart.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-3 text-gray-400">
                ...
                <button onClick={() => setShowCart(false)} className="text-sm text-[#16A34A] font-semibold">Continue shopping</button>
              </div>
            ) : (
              <>
                  ...
                  {cart.map((item) => (
                    <div key={item.product.id} className="flex items-center gap-3 bg-gray-50 rounded-xl p-3">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button onClick={() => updateQuantity(item.product.id, 1)} className="w-7 h-7 rounded-lg bg-[#16A34A] text-white font-bold text-sm flex items-center justify-center">+</button>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="border-t p-4 space-y-3 shrink-0 bg-white">
                  ...
                  <div className="flex justify-between font-bold text-gray-900 text-base border-t pt-2"><span>Total</span><span className="text-[#16A34A]">₱{(cartTotal + cartDeposit).toFixed(2)}</span></div>
          ...
          <div className="relative bg-white rounded-t-3xl md:rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="space-y-3">
                <input placeholder="Full Name" value={checkoutForm.name} onChange={(e) => setCheckoutForm({ ...checkoutForm, name: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-[#16A34A] outline-none" />
                <input type="tel" inputMode="numeric" placeholder="Phone Number" value={checkoutForm.phone} onChange={(e) => setCheckoutForm({ ...checkoutForm, phone: e.target.value.replace(/[^0-9]/g, "") })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-[#16A34A] outline-none" />
                <textarea placeholder="Delivery address..." value={checkoutForm.address} onChange={(e) => setCheckoutForm({ ...checkoutForm, address: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-[#16A34A] outline-none resize-none" rows={2} />
                ...
                {checkoutForm.lat !== 0 && <p className="text-xs text-green-600">✓ Location pinned</p>}
                <input placeholder="Landmark (required)" value={checkoutForm.landmark} onChange={(e) => setCheckoutForm({ ...checkoutForm, landmark: e.target.value })} className={`w-full border rounded-xl px-4 py-3 text-sm focus:border-[#16A34A] outline-none ${!checkoutForm.landmark.trim() ? "border-amber-300 bg-amber-50/30" : "border-gray-200"}`} />
              </div>

              <div className="space-y-2">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Payment</p>
                {paymentMethods.cod && (
                  <label className={`flex items-center gap-3 border-2 rounded-xl px-4 py-3 cursor-pointer ${checkoutForm.paymentMethod === "cod" ? "border-[#16A34A] bg-green-50" : "border-gray-200"}`}>
                    <input type="radio" name="pay" value="cod" checked={checkoutForm.paymentMethod === "cod"} onChange={() => setCheckoutForm({ ...checkoutForm, paymentMethod: "cod" })} className="accent-[#16A34A]" />
                ...
                {user && paymentMethods.wallet && (
                  <label className={`flex items-center gap-3 border-2 rounded-xl px-4 py-3 cursor-pointer ${checkoutForm.paymentMethod === "wallet" ? "border-purple-500 bg-purple-50" : "border-gray-200"}`}>
                    ...
                    <span className="text-xl">👛</span>
                    <div className="flex-1"><p className="text-sm font-bold text-gray-800">Gruwcer Wallet</p><p className="text-xs text-gray-400">Balance: <span className="font-bold text-green-600">₱{walletBalance.toFixed(2)}</span></p></div>
              ...
              <div className="bg-gray-50 rounded-xl p-3 space-y-1.5">
                ...
                {cartDeposit > 0 && <div className="flex justify-between text-sm text-orange-600"><span>Bottle Deposit</span><span>₱{cartDeposit.toFixed(2)}</span></div>}
                <div className="flex justify-between text-sm"><span className={deliveryFee === 0 ? "text-green-600" : "text-gray-600"}>Delivery</span><span className={deliveryFee === 0 ? "text-green-600 font-bold" : ""}>{deliveryFee === 0 ? "FREE" : `₱${deliveryFee.toFixed(2)}`}</span></div>
                <div className="flex justify-between font-bold text-gray-900 border-t pt-2"><span>Total</span><span className="text-[#16A34A] text-lg">₱{(cartTotal + cartDeposit + deliveryFee).toFixed(2)}</span></div>
              </div>
            </div>
    ...
    <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
      ...
      <div className="p-2.5">
        ...
        <div className="flex items-center gap-1.5 mt-1.5">
          <span className="text-[#16A34A] font-black text-sm">₱{displayPrice.toFixed(2)}</span>
...
function HeroSlider() {
  ...
  const fallback = [
    { id: "1", badge: "Express Delivery", title: "Fast Delivery", highlight: "To Your Doorstep", description: "Fresh groceries delivered same day!", imageUrl: "", bgColor: "#16A34A", link: "/grocery" },
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 999999, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      ...
      <div style={{ position: "relative", background: "#fff", borderRadius: "16px", width: "100%", maxWidth: "22rem", padding: "1.5rem", textAlign: "center" }}>
        <div style={{ width: "56px", height: "56px", background: "#dcfce7", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem" }}>
          <svg style={{ width: "32px", height: "32px", color: "#16A34A" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", background: "#f9fafb", borderRadius: "12px", padding: "0.75rem" }}>
          ...
          <div style={{ flex: 1, textAlign: "left", minWidth: 0 }}>
            ...
            <p style={{ color: "#16A34A", fontWeight: "bold", fontSize: "0.875rem" }}>₱{((product.onSale && product.salePrice ? product.salePrice : product.price) * quantity).toFixed(2)}</p>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", marginTop: "1rem" }}>
          ...
          <button onClick={() => onUpdateQuantity(product.id, 1)} style={{ width: "36px", height: "36px", borderRadius: "50%", background: "#16A34A", color: "#fff", border: "none", fontSize: "1.25rem", cursor: "pointer" }}>+</button>
        </div>
